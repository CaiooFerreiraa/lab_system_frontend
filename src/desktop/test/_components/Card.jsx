...
export default function Card({ elements = [], search = "", onRefresh }) { 
  const filteredItems = elements.filter((sector) => 
    sector.status.toLowerCase().includes(search.toLowerCase()) 
  ); 
...
function InfoCard({data, onRefresh}) { 
  ...
  return ( 
    <>
      {loading && <Load />}
      <div className={`cardMark-desktop ${data.status.toLowerCase()}`}> 
        <div className={`infos`} key={data.modelo}> 
          <span className="value">Modelo</span> 
          <span className="value">{data.modelo}</span> 
        </div>
        <div className={`infos`} key={data.status}> 
